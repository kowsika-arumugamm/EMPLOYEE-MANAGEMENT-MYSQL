...
app.post("/insert", (req, res) => {
    ...
    req.on("end", () => {
        ...
        const checkQuery = "SELECT * FROM employees WHERE empID = ?";
        db.query(checkQuery, [queryObj.empID], (err, result) => {
            if (err) {
                console.error("Error checking empID:", err);
                res.end(`Error checking empID: ${err}</h1></div>`);
            } else if (result.length > 0) {
                // empID already exists
                res.end(`Employee ID ${queryObj.empID} already exists!</h1></div>`);
            } else {
                // empID does not exist, proceed with insertion
                ...
                db.query(insertQuery, values, (err, result) => {
                    if (err) {
                        ...
                        res.end(`Error inserting into employees table: ${err}</h1></div>`);
                    } else {
...
app.get("/select", (req, res) => {
    ...
    db.query("SELECT * from employees;", (err, result) => {
        if (err) {
            ...
            let htmlResponse =
                "<table><tr><th>Name</th><th>Email</th><th>Phone</th><th>Employee ID</th><th>Department</th><th>Date Of Join</th><th>Position</th><th>Years of Experience</th></tr>";

            result.forEach(row => {
                let doj = new Date(row["doj"]);
                let today = new Date();
                let years = Math.floor((today.getTime() - doj.getTime()) / (1000 * 3600 * 24 * 365));
                ...
                if (years > 0) {
                    ...
                    let months = Math.floor((today.getTime() - doj.getTime()) / (1000 * 3600 * 24 * 30));
                    htmlResponse += `
                        <tr>
                            <td>${row["name"]}</td>
                            <td>${row["email"]}</td>
                    ...
                    let days = Math.floor((today.getTime() - doj.getTime()) / (1000 * 3600 * 24));
                    htmlResponse += `
                        <tr>
                            <td>${row["name"]}</td>
                            <td>${row["email"]}</td>
